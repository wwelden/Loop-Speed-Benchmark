...
# Configuration
NUM_RUNS=10
INPUT_FILE="$PROJECT_ROOT/benchmark_input.txt"
OUTPUT_FILE="$PROJECT_ROOT/sorted_data.txt"
RESULTS_DIR="$PROJECT_ROOT/results/merge_sort"
IMPLEMENTATIONS_DIR="$PROJECT_ROOT/algorithms/merge_sort/implementations"
BUILD_DIR="$PROJECT_ROOT/algorithms/merge_sort/build"
...
if [ ! -f "$INPUT_FILE" ]; then
    echo "Input file does not exist. Generating test data..."
...
benchmark_assembly() {
    if [ -d "$IMPLEMENTATIONS_DIR/assembly/src" ]; then
        ...
        if [[ "$OSTYPE" == "darwin"* ]]; then
            # macOS
            nasm -f macho64 "$IMPLEMENTATIONS_DIR/assembly/src/merge_sort.asm" -o "$BUILD_DIR/assembly/merge_sort.o"
            gcc -o "$BUILD_DIR/assembly/merge_sort" "$BUILD_DIR/assembly/merge_sort.o"
        else
            # Linux
            nasm -f elf64 "$IMPLEMENTATIONS_DIR/assembly/src/merge_sort.asm" -o "$BUILD_DIR/assembly/merge_sort.o"
            gcc -no-pie -o "$BUILD_DIR/assembly/merge_sort" "$BUILD_DIR/assembly/merge_sort.o"
        fi

        # Run benchmark
        if [ -f "$BUILD_DIR/assembly/merge_sort" ]; then
            run_benchmark "assembly" "$BUILD_DIR/assembly/merge_sort $INPUT_FILE $OUTPUT_FILE"
        else
...
benchmark_c() {
    if [ -d "$IMPLEMENTATIONS_DIR/c/src" ]; then
        # Build
        mkdir -p "$BUILD_DIR/c"
        gcc -O2 "$IMPLEMENTATIONS_DIR/c/src/merge_sort.c" -o "$BUILD_DIR/c/merge_sort"

        # Run benchmark
        if [ -f "$BUILD_DIR/c/merge_sort" ]; then
            run_benchmark "c" "$BUILD_DIR/c/merge_sort $INPUT_FILE $OUTPUT_FILE"
        else
            echo "C build failed."
        fi
    else
...
benchmark_cpp() {
    if [ -d "$IMPLEMENTATIONS_DIR/cpp/src" ]; then
        ...
        g++ -O2 "$IMPLEMENTATIONS_DIR/cpp/src/merge_sort.cpp" -o "$BUILD_DIR/cpp/merge_sort"

        # Run benchmark
        if [ -f "$BUILD_DIR/cpp/merge_sort" ]; then
            run_benchmark "cpp" "$BUILD_DIR/cpp/merge_sort $INPUT_FILE $OUTPUT_FILE"
        else
            echo "C++ build failed."
        fi
    else
...
benchmark_csharp() {
    if [ -d "$IMPLEMENTATIONS_DIR/csharp/src" ]; then
        ...
        # Run benchmark
        if [ -f "$IMPLEMENTATIONS_DIR/csharp/src/bin/Release/net6.0/merge_sort.dll" ]; then
            run_benchmark "csharp" "dotnet $IMPLEMENTATIONS_DIR/csharp/src/bin/Release/net6.0/merge_sort.dll $INPUT_FILE $OUTPUT_FILE"
        else
            echo "C# build failed."
        fi
    else
...
benchmark_go() {
    if [ -d "$IMPLEMENTATIONS_DIR/go/src" ]; then
        ...
        go build -o "$BUILD_DIR/go/merge_sort" "$IMPLEMENTATIONS_DIR/go/src/merge_sort.go"

        # Run benchmark
        if [ -f "$BUILD_DIR/go/merge_sort" ]; then
            run_benchmark "go" "$BUILD_DIR/go/merge_sort $INPUT_FILE $OUTPUT_FILE"
        else
...
benchmark_java() {
    if [ -d "$IMPLEMENTATIONS_DIR/java/src" ]; then
        ...
        javac -d "$BUILD_DIR/java" "$IMPLEMENTATIONS_DIR/java/src/MergeSort.java"

        # Run benchmark
        if [ -f "$BUILD_DIR/java/MergeSort.class" ]; then
            run_benchmark "java" "java -cp $BUILD_DIR/java MergeSort $INPUT_FILE $OUTPUT_FILE"
        else
...
benchmark_javascript() {
    if [ -d "$IMPLEMENTATIONS_DIR/javascript/src" ]; then
        ...
        if [ -f "$IMPLEMENTATIONS_DIR/javascript/src/merge_sort.js" ]; then
            run_benchmark "javascript" "node $IMPLEMENTATIONS_DIR/javascript/src/merge_sort.js $INPUT_FILE $OUTPUT_FILE"
        else
...
benchmark_typescript() {
    if [ -d "$IMPLEMENTATIONS_DIR/typescript/src" ]; then
        ...
        tsc --outDir "$BUILD_DIR/typescript" "$IMPLEMENTATIONS_DIR/typescript/src/merge_sort.ts"

        # Run benchmark
        if [ -f "$BUILD_DIR/typescript/merge_sort.js" ]; then
            run_benchmark "typescript" "node $BUILD_DIR/typescript/merge_sort.js $INPUT_FILE $OUTPUT_FILE"
        else
...
benchmark_python() {
    if [ -d "$IMPLEMENTATIONS_DIR/python/src" ]; then
        ...
        if [ -f "$IMPLEMENTATIONS_DIR/python/src/merge_sort.py" ]; then
            run_benchmark "python" "python3 $IMPLEMENTATIONS_DIR/python/src/merge_sort.py $INPUT_FILE $OUTPUT_FILE"
        else
...
benchmark_bash() {
    if [ -d "$IMPLEMENTATIONS_DIR/bash/src" ]; then
        ...
        if [ -f "$IMPLEMENTATIONS_DIR/bash/src/merge_sort.sh" ]; then
            # Make sure it's executable
            chmod +x "$IMPLEMENTATIONS_DIR/bash/src/merge_sort.sh"
            run_benchmark "bash" "bash $IMPLEMENTATIONS_DIR/bash/src/merge_sort.sh $INPUT_FILE $OUTPUT_FILE"
        else
...
benchmark_rust() {
    if [ -d "$IMPLEMENTATIONS_DIR/rust/src" ]; then
        ...
        # Run benchmark
        if [ -f "$IMPLEMENTATIONS_DIR/rust/target/release/merge_sort" ]; then
            run_benchmark "rust" "$IMPLEMENTATIONS_DIR/rust/target/release/merge_sort $INPUT_FILE $OUTPUT_FILE"
        else
...
benchmark_swift() {
    if [ -d "$IMPLEMENTATIONS_DIR/swift/src" ]; then
        ...
        swiftc -O "$IMPLEMENTATIONS_DIR/swift/src/merge_sort.swift" -o "$BUILD_DIR/swift/merge_sort"

        # Run benchmark
        if [ -f "$BUILD_DIR/swift/merge_sort" ]; then
            run_benchmark "swift" "$BUILD_DIR/swift/merge_sort $INPUT_FILE $OUTPUT_FILE"
        else
